...
**Project child notes:** Any note or task that belongs to a project gets `project: "[[Project MOC Name]]"` in frontmatter. Place it after `subjects:`. The wikilink makes the note appear in the MOC's Project Files base view (`file.hasLink(this.file)` filter). Leave `project:` blank when the note has no project parent. Reference templates (framework, cheatsheet, setup, audit) and permanent notes templates (concept, writeup) include the field by default.
